...
mp.events.add("playerChat", (player, text) => {
  mp.players.forEach((target) =>
    target.sendClientMessage(`${player.name} says: ${text}`)
  );
});
...
mp.events.add("playerReady", (player, text) => {
  ...
  player.sendClientMessage(
    `!{#dddddd}Welcome to our modding server. Use !{#ffffff}/help !{#dddddd} for commands.`
  );
});

mp.events.add("playerDeath", (player) => {
  player.spawn(new mp.Vector3(SPAWN_SERVER.x,
    SPAWN_SERVER.y,
    SPAWN_SERVER.z));
