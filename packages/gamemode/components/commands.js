...
const { lastClothesIds, arrayClothing } = require('../data/lastClothes');
...
mp.events.addCommand('setclothes', (player, fullText, type, drawable, texture) => {
  if (!fullText || !type || !drawable || !texture) return player.sendClientMessage(`Args: /setclothes [component id] [drawable] [texture]`);
...
mp.events.addCommand('setaclothes', (player, fullText, type, drawable, texture) => {
  if (!fullText || !type || !drawable || !texture) return player.sendClientMessage(`Args: /setaclothes [component id] [drawable] [texture]`);

  const gender = player.model === mp.joaat('mp_m_freemode_01') ? "male" : "female";

  const typeMatch = arrayClothing.find((c) => !c.isProp && c.componentId === parseInt(type));

  const lastDrawableId = lastClothesIds[gender][typeMatch?.pluralKey];

  if (!lastDrawableId) return player.sendClientMessage("Error: Failed to find matching last drawable id. Check type?");

  const drawableIdFinal = parseInt(drawable) + lastDrawableId;

  player.setClothes(parseInt(type), drawableIdFinal, parseInt(texture), 0);

  player.sendClientMessage(`Addon Clothes applied: type ${type}, drawable id ${drawableIdFinal}, texture id ${texture}`);

});


mp.events.addCommand('setprops', (player, fullText, type, drawable, texture) => {
  if (!fullText || !type || !drawable || !texture) return player.sendClientMessage(`Args: /setprops [type] [drawable] [texture]`);
...
mp.events.addCommand('setaprops', (player, fullText, type, drawable, texture) => {
  if (!fullText || !type || !drawable || !texture) return player.sendClientMessage(`Args: /setaprops [type] [drawable] [texture]`);
  ...
  player.setProp(parseInt(type), parseInt(drawableIdFinal), parseInt(texture));

  player.sendClientMessage(`Addon Props applied: type ${type}, drawable id ${drawableIdFinal}, texture id ${texture}`);
});
